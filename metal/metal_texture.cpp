...
MetalTexture::MetalTexture(MTL::Device *device, PixelFormat format, uint dimension,
    ...
    switch (dimension) {
        ...
        default: ERROR_WITH_LOCATION("Invalid MetalTexture dimension {}.", dimension);
    }
    switch (format) {
...
void MetalTexture::set_name(std::string_view name) noexcept {
    auto n = _maps[0u]->mipmapLevelCount();
    if (name.empty()) {
        for (auto i = 0u; i < n; i++) {
        ...
        for (auto i = 0u; i < n; i++) {
            auto level_name = fmt::format("{} (level {})", name, i);
