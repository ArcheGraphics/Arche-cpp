...
struct Node {
    ...
    Node(int id, const SdfPath &path, const ImVec2 &pos, float value, const ImVec4 &color, int inputs_count, int outputs_count) {
        ID = id;
        Path = path;
        Pos = pos;
        Value = value;
...
int _WalkGraph(UsdShadeConnectableAPI const &shadeNode,
    ...
    if (nodeLayout.size() <= depth) {
        nodeLayout.push_back(1);
    } else {
        nodeLayout[depth]++;
    }
...
static void ShowExampleAppCustomNodeGraph(const UsdPrim &prim) {
    ...
    if (ImGui::IsWindowHovered()
        ...
        && ImGui::IsMouseDragging(ImGuiMouseButton_Left, 0.f) && ImGui::IsKeyDown(ImGuiKey_Space)) {
