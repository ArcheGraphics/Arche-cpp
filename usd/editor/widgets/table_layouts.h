...
inline bool HasEdits(const Args &...args) { return true; }

template<typename FieldT, typename... Args>
inline ScopedStyleColor GetRowStyle(const int rowId, const Args &...args) {
...
//
template<typename FieldT, typename... Args>
inline void DrawFirstColumn(const int rowId, const Args &...args) {}

template<typename FieldT, typename... Args>
inline void DrawSecondColumn(const int rowId, const Args &...args) {
    ImGui::Text(FieldT::fieldName);
}
template<typename FieldT, typename... Args>
inline void DrawThirdColumn(const int rowId, const Args &...args) {}

//
// Provides a standard/unified 3 columns layout
//
template<typename FieldT, typename... Args>
inline void DrawThreeColumnsRow(const int rowId, const Args &...args) {
...
template<typename FieldT, typename... Args>
inline void DrawTwoColumnsRow(const int rowId, const Args &...args) {
    ImGui::TableNextRow(ImGuiTableRowFlags_None, TableRowDefaultHeight);
    ScopedStyleColor style = GetRowStyle<FieldT>(rowId, args...);
    ImGui::TableSetColumnIndex(0);
    DrawFirstColumn<FieldT>(rowId, args...);
