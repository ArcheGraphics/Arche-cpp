...
private:
    ...
    [[nodiscard]] virtual uint64_t create_device_capture_scope(std::string_view label, const Option &option) const noexcept = 0;
    [[nodiscard]] virtual uint64_t create_stream_capture_scope(uint64_t stream_handle, std::string_view label, const Option &option) const noexcept = 0;
...
public:
    [[nodiscard]] auto create_scope(std::string_view label, const Option &option = {}) const noexcept {
        return DebugCaptureScope{this, create_device_capture_scope(label, option)};
    }
    [[nodiscard]] auto create_scope(std::string_view label, const Stream &stream, const Option &option = {}) const noexcept {
